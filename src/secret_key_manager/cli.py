...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    providers_writable_parser = providers_subparsers.add_parser(
        "writable", 
...
def handle_set_command(args: argparse.Namespace) -> int:
    ...
    if persist:
        ...
        if not writable_providers:
            ...
            if args.provider:
                logger.error(f"Specified providers {args.provider} do not support writing")
                return 1
            else:
    ...
    # Set the key
    result = keys.set_key(args.key_name, args.key_value, persist=persist, providers=args.provider)
    
    # Report success or failure
    if result or not persist:
        print(f"Key '{args.key_name}' set successfully")
        if persist:
            print(f"Key persisted to providers: {', '.join(args.provider) if args.provider else 'all writable providers'}")
        else:
...
def handle_providers_status_command(args: argparse.Namespace) -> int:
    ...
    print(f"  {'PROVIDER':{max_name_length}}  {'PRIORITY':<10}  {'STATUS':<10}  {'WRITABLE':<10}  {'CLASS'}")
    ...
    for name, info in sorted_providers:
        ...
        print(f"  {name:{max_name_length}}  {info['priority']:<10}  {status:<10}  {writable:<10}  {info['class']}")
