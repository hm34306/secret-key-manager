...
class DotEnvProvider:
    ...
    def __init__(self, file_path: str = ".env"):
        ...
        if not HAS_DOTENV:
            logger.warning(
                "python-dotenv package is not installed. DotEnv provider will not work."
            )
    ...
    def write_key(self, key_name: str, key_value: str, **kwargs) -> bool:
        ...
        file_path = os.path.abspath(
            os.path.expanduser(kwargs.get("file_path", self.file_path))
        )
        ...
        try:
            ...
            logger.debug(
                f"Successfully wrote key '{key_name}' to .env file {file_path}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error writing key '{key_name}' to .env file {file_path}: {e}"
            )
            return False
