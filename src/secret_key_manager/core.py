...
class KeyManager:
    ...
    def get_provider_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for provider_info in _PROVIDER_REGISTRY.values():
            ...
            provider_instance = next((p for p in self._providers if p.name == provider_name), None)
            ...
            if provider_instance:
                ...
                if hasattr(provider_instance, 'get_provider_info'):
                    try:
                        ...
                        logger.warning(f"Error getting provider info for '{provider_name}': {e}")
    ...
    def set_key(self, key_name: str, key_value: str, persist: bool = False, providers: Optional[List[str]] = None) -> bool:
        """
        ...
        for provider in active_providers:
            ...
            if hasattr(provider, 'validate_key') and not provider.validate_key(key_name, key_value):
                logger.warning(f"Key '{key_name}' failed validation for provider '{provider.name}'")
                continue
                
            # Try to write the key
            try:
                if provider.write_key(key_name, key_value):
                    logger.debug(f"Successfully wrote key '{key_name}' to provider '{provider.name}'")
                    success = True
            except Exception as e:
                logger.warning(f"Error writing key '{key_name}' to provider '{provider.name}': {e}")
