...
from secret_key_manager.providers.file import FileKeyProvider
...
def test_file_provider():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Test key that exists
        assert provider.get_key("test_key") == "secret-value"
    
    # Verify provider name
    assert provider.name == "file"


# Test file provider with nested directories
def test_file_provider_nested_dirs():
    provider = FileKeyProvider()
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        nested_dir = os.path.join(temp_dir, "api", "services")
        os.makedirs(nested_dir, exist_ok=True)
        
        # Create a test key file in the nested directory
        key_path = os.path.join(nested_dir, "nested_key")
        with open(key_path, "w") as f:
            f.write("nested-secret-value")
        
        # Test accessing the nested key
        assert provider.get_key("api/services/nested_key") == "nested-secret-value"


# Test 1Password provider
@patch('subprocess.run')
@patch('os.path.expanduser')
@patch('builtins.open', new_callable=mock_open, read_data='{"account": "test.1password.com", "env_file": "~/.local/.env"}')
...
def test_dotenv_provider():
    ...
    with tempfile.NamedTemporaryFile(suffix='.env', mode='w+') as temp_env:
        ...
        with patch('dotenv.load_dotenv', return_value=True), \
             patch('dotenv.dotenv_values', return_value={"DOTENV_TEST_KEY": "dotenv-secret-value"}), \
